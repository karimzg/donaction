		...
		for (const pattern of SECURITY_RULES.DANGEROUS_PATTERNS) {
			if (pattern.test(command)) {
                ...
                for (const path of SECURITY_RULES.PROTECTED_PATHS) {
                        ...
                        const escapedPath = pathWithoutTrailingSlash.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                        const absolutePathRegex = new RegExp(`(^|[\\s\"'=><|;&])${escapedPath}(?:/|$)`, "i");
                        ...
                        if (absolutePathRegex.test(command)) {
                                ...
                                result.isValid = false;
                                result.severity = "HIGH";
                                result.violations.push(`Access to protected system path: ${path}`);
                        }
                }
