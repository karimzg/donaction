...
- **Directory Structure**:
    - `donaction-frontend`: Frontend project folder.
    - `donaction-saas`: SaaS project folder.
    - `public/donaction-web-components`: Destination folder for synced web components.
...
### 3. API Request
The script sends a POST request to http://localhost:1437/api/klubr-subscriptions to fetch a subscription token using the provided NEXT_PUBLIC_STRAPI_API_TOKEN. The request is made using the curl tool.
...
Installs Dependencies: Runs npm install in the donaction-saas folder.
Builds Components: Runs npm run build in the donaction-saas folder to generate the web components.
Syncs Built Files: Deletes the existing public/donaction-web-components folder, creates a new one, and copies the built components into it.
