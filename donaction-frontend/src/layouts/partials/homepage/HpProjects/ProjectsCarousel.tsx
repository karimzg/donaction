...
export default function ProjectsCarousel({ projets }: ProjectsCarouselProps) {
  ...
  return (
    <div className="hp-projects__carousel relative">
      <div
        ref={scrollRef}
        className="hp-projects__carousel-track flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 -mx-6 px-6"
        role="region"
      ...
      >
        {projets.map((projet, index) => (
          <div
            ...
            className="hp-projects__carousel-slide flex-none w-[82%] snap-start"
          >
            <HpProjectCard projet={projet} index={index} />
          </div>
        ))}
      </div>
      ...
      {projets.length > 1 && (
        <div className="flex items-center justify-center gap-4 mt-4">
          {/* Prev arrow */}
          <button
            className={`hp-projects__arrow flex items-center justify-center w-8 h-8 rounded-full border border-gray-300 transition-all duration-200 ${
              canGoPrev
          ...
          <div className="flex gap-2" role="tablist" aria-label="Navigation projets">
            {projets.map((_, index) => (
              <button
                key={index}
                className={`hp-projects__dot w-2 h-2 rounded-full transition-all duration-300 ${
                  index === activeIndex
                    ? 'bg-gray-900 w-6'
                    : 'bg-gray-300'
                }`}
