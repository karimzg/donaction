...
import PaginatedProjectsList from 'src/layouts/partials/common/paginatedProjectsList';
...
export default async function page() {
	...
	const result = await getProjets(1, 10, false, !!isPreview, cookies().toString()).catch((error) =>
		notFound(),
	);
	if (!result) {
		notFound();
		...
		<div className='text-black py-10 z-10 bg-white w-full min-h-[25rem] flex flex-col items-center'>
			...
			<div className='minMaxWidth flex flex-col gap-6'>
				...
				<Link href={`/`} className='flex items-center gap-2 px-6 md:px-0'>
					...
					<p className='font-bold md:text-3xl text-xl'>Découvrez les projets :</p>
				</Link>
